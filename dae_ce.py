...
class DAE(nn.Module):
    ...
    def fit(self, train_X, test_X, lr=0.001, batch_size=256, epoches=10, momentum=0.1):
        # loss_fn = nn.BCELoss()
        loss_fn = nn.MSELoss()
        print('d={:d}  ratio={:.2f}  lr={:.3f}  bs={:d}  epoches={:d}  momentum={:.2f}  loss={:s}' .format(self.out_features, self.p_dropout, lr, batch_size, epoches, momentum, str(loss_fn)))
        optimizer = torch.optim.SGD(self.parameters(), lr=lr, momentum=momentum) # 
...
for di in range(len(drop_arr)):
	...
	for target_dim in range(d_lb,d_ub):
	    ...
	    torch.save(dae.state_dict(), "./model/sdaeinit_dae_drop"+str(p_dropout)+"_d"+str(target_dim)+"_eph"+str(epoches)+"_lay"+str(len(dae.encoder)/2)+".pth")
